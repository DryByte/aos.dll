...
void draw_to_buffer(struct Menu* menu, int* copy_buff, int offset_x, int offset_y, int size_x, int size_y) {
	...
	for (int x = 0; x < size_x; x++) {
		for (int y = 0; y < size_y; y++) {
			*((menu->buffer)+x+offset_x+(y+offset_y)*menu->buffer_y) = *(copy_buff+(x+y*size_x));
		}
	}

	drawtile((long)menu->buffer, 1, 1, 1, 0x0, 0x0, menu->x_pos, menu->y_pos, menu->x_size, menu->y_size, -1);
}
